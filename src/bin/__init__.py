        ...
        if message.photo:
            ...
            try:
                ...
                await discord_channel.send(
                    f"**{sender}:** {text}",
                    file=discord.File(re_encoded_image, filename="photo.jpg"),
                )
            except (ValueError, OSError) as e:
                logger.error(f"Error sending photo to Discord: {e}")
                await discord_channel.send(
                    f"**{sender}:** Photo could not be sent. {text}"
                )
        else:
...
async def get_chat_id(update, context):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Chat ID: {chat_id}"
    )

    if TELEGRAM_GROUP_ID is None:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Please set the TELEGRAM_GROUP_ID environment variable to {chat_id} and restart the bot.",
        )
...
async def on_message(message):
    ...
    if (
        ...
        and message.author != discord_bot.user
        and TELEGRAM_GROUP_ID is not None
    ):
                        ...
                        async with session.get(attachment.url) as resp:
                            if resp.status == 200:
                                ...
                                await telegram_app.bot.send_photo(
                                    ...
                                    photo=image_bytes,
                                    caption=f"**{message.author.name}:** {message.content}",
                                )
                            else:
                                logger.error(
                                    f"Failed to download image from Discord: {resp.status}"
                                )
                else:
